...
export default function VendorProfileVerificationPage() {
  ...
  const statusBadge = useMemo(() => {
    ...
    return <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${className}`}>{status}</span>;
  }, [verification?.status]);
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        toast.error(error?.response?.data?.message || 'Failed to load verification');
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // Upload CAC file first if user selected a new one.
      if (cacFile) {
        ...
        // Route returns { url } or { secure_url/url }
        cacDocumentUrl = up.data?.url || up.data?.secure_url || up.data?.cacDocumentUrl;
      }
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {isRejected && (
            <div className="mb-6 rounded-xl border border-rose-200 bg-rose-50 p-4">
              <p className="text-rose-800 font-semibold text-sm">Rejected — please update your details and resubmit.</p>
              {verification?.adminNotes ? (
                <p className="text-rose-700 text-xs mt-2 whitespace-pre-wrap">{verification.adminNotes}</p>
              ) : null}
            </div>
          )}
              ...
              <div className="md:col-span-2">
                ...
                {verification?.cacDocumentUrl && !cacFile ? (
                  <p className="text-xs text-slate-500 mt-2">
                    Existing document is already uploaded. Upload a new file to replace it.
                  </p>
                ) : null}
              </div>
            </div>
