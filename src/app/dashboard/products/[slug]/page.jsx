    ...
    return (
        <div className="min-h-screen bg-white text-black antialiased selection:bg-black selection:text-white">
            <header className="border-b border-black py-6 px-6 md:px-12 flex justify-between items-baseline">
                <button onClick={() => router.back()} className="text-md font-semibold  uppercase tracking-widest opacity-60 hover:opacity-100 transition-opacity">
                    ←    Back
                </button>
            ...
            <main className="grid grid-cols-1 md:grid-cols-2 min-h-[calc(100vh-69px)]">
                ...
                <div className="p-8 md:p-12 flex flex-col justify-between">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="border-t border-b border-black py-4 flex items-baseline gap-4 ">
                            {product?.discountPrice ? (
                                <>
                                    <span className="text-2xl font-black text-black">{formatPrice(product?.discountPrice)}</span>
                    ...
                    <div className="mt-12 space-y-3">
                        <button
                            ...
                            className="w-full bg-black text-white text-xs font-bold uppercase tracking-widest py-4 hover:bg-neutral-800 transition-colors duration-200"
                        >
            ...
            <EditProductModal
                ...
                onSave={async (fields) => {
                    try {
                        const res = await api.put(`/products/${product._id}`, fields);
                        if (res.data.success) {
